...
class ProjectGenerator {
  ...
  Future<void> generate() async {
    ...
    final result = await Process.run(flutterCmd, ['create', projectName, '--empty'], runInShell: true);
    ...
    print('Project "\$projectName" created. Run `cd \$projectName` then `flutter pub get`.');
  }
  ...
  Future<void> _createTemplates() async {
    final lib = p.join(projectName, 'lib');
    await _createFile(p.join(lib, 'core', 'either', 'either.dart'), eitherTemplate);
    await _createFile(p.join(lib, 'core', 'service_locator', 'service_locator.dart'), serviceLocatorTemplate);
    await _createFile(p.join(lib, 'core', 'routes', 'app_routes.dart'), routesTemplate);
    await _createFile(p.join(lib, 'core', 'theme', 'app_theme.dart'), appThemeTemplate);
    ...
    await _createFile(p.join(projectName, 'MVVM_README.md'), mvvmReadmeTemplate);
    await _createFile(p.join(projectName, 'analysis_options.yaml'), mvvmAnalysisTemplate);
  }
